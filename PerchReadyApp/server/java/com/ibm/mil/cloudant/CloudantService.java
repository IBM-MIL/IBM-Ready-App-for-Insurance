...
public final class CloudantService {
	...
	private void connect() {
		CloudantClient cloudant = new CloudantClient(
				AppPropertiesReader.getStringProperty(Constants.ACCOUNT_KEY),
				AppPropertiesReader.getStringProperty(Constants.USERNAME_KEY),
				AppPropertiesReader.getStringProperty(Constants.PASSWORD_KEY));
		db = cloudant.database(AppPropertiesReader.getStringProperty(Constants.DB_KEY), true);
	}
	...
	private List<Map> getMapCollection(String viewName, Object[] startKeys, Object... endKeys) {
		...
		View view = db.view(
				AppPropertiesReader.getStringProperty(Constants.MAIN_DESIGN_DOC) + "/" + viewName)
				.includeDocs(true);
